...
unsigned param_check(char * param_str,unsigned * param_ptr,int param_type,int count)
{
    ...
    if(return_code == 0)
    {
        ...
        if( ((*param_ptr >= BANK_A_USER_START_SECTOR_ADDRESS) && ((*param_ptr+count) <= (BANK_A_USER_END_SECTOR_ADDRESS+1))) )
        {
            in_flash = TRUE;
        }
        else if( ((*param_ptr >= BANK_B_USER_START_SECTOR_ADDRESS) && ((*param_ptr+count) <= (BANK_B_USER_END_SECTOR_ADDRESS+1))) )
        {
            in_flash = TRUE;
        }
        else
...
void copy(void)
{
    ...
    */
        ...
        if (crp_after_reset == CRP1)
        {
            if( (param_table[IAP_PARAM_0] < (BANK_A_USER_START_SECTOR_ADDRESS+BANK_A_USER_START_SECTOR_SIZE)) )
            {
        ...
        if( result_table[IAP_STAT_CODE] == CMD_SUCCESS )
        {
            ...
            param_table[IAP_PARAM_3] = SystemCoreClock / 1000;
            iap_entry(param_table,result_table);
        }
        sendline_crlf(i_to_a(result_table[IAP_STAT_CODE],param_buf[ISP_PARAM_1], \
        PARAM_SIZE));
...
void erase(void)
{
    ...
    */
        ...
        if( result_table[IAP_STAT_CODE] == CMD_SUCCESS )
        {
            ...
            {
                result_table[IAP_STAT_CODE] = param_check(param_buf[ISP_PARAM_3], \
                &param_table[IAP_PARAM_3],NO_PARAM_CHECK,NUL);
            }
        }
        ...
        {
            // erase all of bank a is ok
            if( (param_table[IAP_PARAM_0] == BANK_A_USER_START_SECTOR) &&                         \
                (param_table[IAP_PARAM_1] == BANK_A_USER_END_SECTOR)
              )
            {
            }
            // erase all of bank b is ok
            else if( (param_table[IAP_PARAM_0] == BANK_B_USER_START_SECTOR) &&                         \
                     (param_table[IAP_PARAM_1] == BANK_B_USER_END_SECTOR)
              )
            {
            }
            // anything else is a partial erase, which we don't allow
            else
            {
                result_table[IAP_STAT_CODE] = CODE_READ_PROTECTION_ENABLED;
            }
        ...
        if( result_table[IAP_STAT_CODE] == CMD_SUCCESS )
        {
            ...
            param_table[IAP_PARAM_2] = SystemCoreClock / 1000;
            iap_entry(param_table,result_table);
        }
...
void execute_user_code(void)
{
    ...
    user_code_entry = (void (*)(void))(BANK_A_USER_START_SECTOR_ADDRESS | 0x00000001);
    user_code_entry();
}
...
{
    param_table[IAP_CMD_CODE] = BLANK_CHECK_SECTOR;
    param_table[IAP_PARAM_0] = BANK_A_USER_START_SECTOR;
    param_table[IAP_PARAM_1] = BANK_A_USER_START_SECTOR;
    // we only check bank A for user code
    param_table[IAP_PARAM_2] = 0;
    iap_entry(param_table,result_table);
	if( result_table[IAP_STAT_CODE] == CMD_SUCCESS )
	{
	    return (FALSE);
...
{
  Chip_SCU_PinMux(ISP_ENTRY_IOPORT, ISP_ENTRY_IOPIN, SCU_MODE_INBUFF_EN | SCU_MODE_INACT, SCU_MODE_FUNC0); 
...
void exec_isp_cmd(int no_of_param)
{
    ...
    if(c != NUL);
    {
        ...
        case 'N':
            ...
            sendline_crlf(i_to_a(result_table[IAP_RESULT_1],param_buf[ISP_PARAM_1], \
            PARAM_SIZE));
            sendline_crlf(i_to_a(result_table[IAP_RESULT_2],param_buf[ISP_PARAM_1], \
            PARAM_SIZE));
            sendline_crlf(i_to_a(result_table[IAP_RESULT_3],param_buf[ISP_PARAM_1], \
            PARAM_SIZE));
        break;
